...
* **Forward ledger (Chart D, `sim/evolution/vintage_ledger.py`):** the strongest learning measure. Champions are frozen with the timestamp of the last candle they ever saw, then re-tested exclusively on candles that arrived later — data that could not possibly have been overfit. Each week of champions is scored as a percentile against 20 random strategies frozen the same day (plus buy-and-hold and SMA-cross baselines), so market regime is controlled for. Rising percentile = genuine intelligence gain; months near the 50th percentile = no persistent edge with the current feature set, i.e. invest in new data sources rather than more search.
